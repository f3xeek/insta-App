...
const user = {
    ...
    actions: {
        ...
        async UPLOAD_IMAGE_WITH_TAGS({ commit, getters, dispatch }, payload) {
            ...
            const responseFile = await postFile(
                formadata,
                getters.GET_CURRENT_USER_TOKEN
            );
            if (responseFile.status == "error") {
                alert(responseFile.message);
            } else {
                const responseTags = await massTags(
                    responseFile.data.id,
                    payload.tags,
                    getters.GET_CURRENT_USER_TOKEN
                );
        ...
        async PATCH_USER_DATA({ commit, getters }, payload) {
            commit("SET_CURRENT_USER_LOADING", true);
            const response = await patchProfileEdit(
                payload,
                getters.GET_CURRENT_USER_TOKEN
            );
